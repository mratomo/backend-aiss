...
# Importación de FastMCP con manejo de diferentes versiones
try:
    # Intenta importar la versión más reciente
    from fastmcp import mount_mcp
    use_mount_func = True
except ImportError:
    try:
        # Fallback a la versión anterior
        from fastmcp import MountMCP
        use_mount_func = False
    except ImportError:
        # Implementación de fallback si no se puede importar
        def MountMCP(app, server, route=None):
            app.include_router(server.router, prefix=route)
        use_mount_func = False
...
if use_mount_func:
    mount_mcp(app, mcp_service.server, settings.mcp.api_route)
else:
